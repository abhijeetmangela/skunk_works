...
class UAVPerformanceCalculator:
    def __init__(self):
        ...
        # ==========================================
        # Replace these with your actual UAV parameters
        self.rho = 1.142  # Air density at cruise altitude (kg/m^3)
    ...
    def plot_graphs(self):
        ...
        # 4. V-n Diagram
        V_stall_pos = np.sqrt(
            (2 * self.W_o * self.n_max) / (self.rho * self.S_ref * self.CL_max)
        )
        V_stall_neg = np.sqrt(
            (2 * self.W_o * abs(self.n_min))
            / (self.rho * self.S_ref * abs(self.CL_min))
        )

        v_aerodynamic_pos = np.linspace(0, V_stall_pos, 50)
        n_aerodynamic_pos = (
            0.5 * self.rho * (v_aerodynamic_pos**2) * self.S_ref * self.CL_max
        ) / self.W_o

        v_aerodynamic_neg = np.linspace(0, V_stall_neg, 50)
        n_aerodynamic_neg = (
            -(0.5 * self.rho * (v_aerodynamic_neg**2) * self.S_ref * abs(self.CL_min))
            / self.W_o
        )

        plt.figure(figsize=(10, 6))
        # Aerodynamic limits
        plt.plot(v_aerodynamic_pos, n_aerodynamic_pos, "b-", linewidth=2)
        plt.plot(v_aerodynamic_neg, n_aerodynamic_neg, "b-", linewidth=2)
        # Structural limits
        plt.hlines(
            self.n_max, V_stall_pos, 40, colors="b", linestyles="solid", linewidth=2
        )
        plt.hlines(
            self.n_min, V_stall_neg, 40, colors="b", linestyles="solid", linewidth=2
        )
        # Close the envelope on the right (Assume max speed limit V_D = 40 for plotting)
        plt.vlines(
            40, self.n_min, self.n_max, colors="b", linestyles="solid", linewidth=2
        )
        ...
        plt.grid(True)
        print(f"\nPositive Corner Velocity (V*): {V_stall_pos:.2f} m/s")
        plt.show()
